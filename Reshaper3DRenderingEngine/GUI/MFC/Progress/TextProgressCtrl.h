...
#ifdef _HYEXT
	class __declspec(dllexport) CTextProgressCtrl : public CProgressCtrl
#else
	class __declspec(dllimport) CTextProgressCtrl : public CProgressCtrl
#endif
{
	...
	public:
		inline void SetReal(const BOOL iSet) { m_Real = iSet; }
		inline void SetComment(LPCTSTR lpComment) { m_Comment = lpComment; if (!m_Comment || m_Comment.IsEmpty()) ClearComment(); }
		void ClearComment();
